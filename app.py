...
def create_trained_model(input_shape, epoch=20):
...
def test(model_name, input_shape):
    handgestureregconition.run(model_name, input_shape=input_shape)
...
def create_classifier_finger(finger, epoch, input_shape):
...
def test_model(model_type, input_shape, test_data_dir='test'):
    acc = []
    for finger in range(0, 5):
        img_count = 0
        right_ans = 0
        model = cnnmodel.load_classifier(finger, model_type=model_type)
        for label in os.listdir(test_data_dir):
            ...
            for img in os.listdir(os.path.join(test_data_dir, label)):
                ...
                yhat = cnnmodel.predict_image(model, cv2.cvtColor(cv2.imread(os.path.join(test_data_dir, label, img)),
                                                                  cv2.COLOR_BGR2RGB), input_shape=input_shape)
                if (yhat - 0.5) * (y - 0.5) > 0:
                    right_ans += 1
...
if __name__ == "__main__":
    ...
    # check_data(0)
    # for i in range(0, 5):
    #     create_classifier_finger(i, 20,input_shape=input_shape)
    # create_classifier_finger(0, 20,input_shape=input_shape)

    # continue_training_classifier(0,100)
    # create_img_data('11111', 100, data_dir='seen_test')
    test('inception_0.h5',input_shape=input_shape)

    # test_model(model_type='inception_',input_shape=input_shape,test_data_dir='test')

    # model = cnnmodel.load_model('2.h5')
    # cnnmodel.predict(model,os.path.join('data1/01110','100.jpeg'))

    # a = test_model(model_type='resnet_v2_', test_data_dir='seen_test', input_shape=input_shape)
    # b = test_model(model_type='resnet_v2_', test_data_dir='test', input_shape=input_shape)
    #
    # print(a)
    # print(b)
    # run_model()
